...
export const getFacebookEmbedUrl = (url) => {
...
export const InstagramEmbed = Node.create({
    ...
    addCommands() {
        return {
            setInstagramEmbed: (options) => ({ commands }) => {
...
export const getTwitterUsername = (url) => {
    if (!url)
        return null;
    const regExp = /(?:twitter\.com|x\.com)\/(?:#!\/)?(\w+)\/status(?:es)?\/(\d+)/;
    const match = url.match(regExp);
...
export const TwitterEmbed = Node.create({
    ...
    addCommands() {
        return {
            setTwitterEmbed: (options) => ({ commands }) => {
...
export const FacebookEmbed = Node.create({
    ...
    renderHTML({ HTMLAttributes }) {
        ...
        return [
            'div',
            mergeAttributes({ 'data-facebook-video': '' }, this.options.HTMLAttributes, {
    ...
    addCommands() {
        return {
            setFacebookEmbed: (options) => ({ commands }) => {
