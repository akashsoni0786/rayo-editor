...
// --- Styles ---
import "../link-popover/link-popover.scss"
...
export const useLinkHandler = (props: LinkHandlerProps) => {
  ...
  const setLink = React.useCallback(() => {
    ...
    // Add small delay to prevent immediate reopening due to link becoming active
    setTimeout(() => {
      onSetLink?.()
    }, 50)
...
export function LinkPopover({
  ...
  const [justSetLink, setJustSetLink] = React.useState(false)

  const onSetLink = () => {
    setIsOpen(false)
    setJustSetLink(true)
    // Reset flag after a short delay
    setTimeout(() => setJustSetLink(false), 200)
  }

  const onLinkActive = () => {
    // Only auto-open if we didn't just set a link
    if (autoOpenOnLinkActive && !justSetLink) {
      setIsOpen(true)
    }
  }
