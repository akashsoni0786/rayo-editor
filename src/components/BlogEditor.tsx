...
export const BlogEditor: React.FC<BlogEditorProps> = ({
  ...
  readOnly = false,
  onAriScoreChange,
  disableAutoScroll: _disableAutoScroll = false,
  onUserScrollChange,
  ...
  const [, forceTextHoverUpdate] = useState(0);
  ...
  useEffect(() => {
    ...
    let imageHideTimeout: ReturnType<typeof setTimeout> | null = null;
      ...
      <div className={`flex-1 w-full relative overflow-hidden flex flex-col pt-5  ${isReviewMode ? 'blog-editor-review-mode' : ''} ${isStreaming ? 'blog-editor-streaming' : ''} ${showToolbarAnimation ? 'blog-editor-toolbar-slide-in' : ''} ${focusMode ? 'blog-editor-focus-mode' : ''}`}>
        <div className='flex-1 flex flex-col overflow-hidden w-full'>
          <BlogSimpleEditor
            ...
            height="100%" className={`h-full w-full blog-editor-styled ${!title ? 'no-title' : ''}`}
            documentTitle={title} animated={true}
            readOnly={readOnly} isStreaming={Boolean(isStreaming)}
            disableAutoScroll={_disableAutoScroll} onUserScrollChange={onUserScrollChange}
            titleElement={
