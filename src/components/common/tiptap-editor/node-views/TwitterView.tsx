...
// Declare global twttr object for TypeScript
declare global {
  interface Window {
    twttr?: {
      widgets: {
...
function TwitterView({ node, selected }: NodeViewProps) {
  ...
  if (hasError || !tweetId) {
    return (
      <NodeViewWrapper className="embed-node-wrapper">
        <div
          className={`twitter-embed-container twitter-fallback ${selected ? 'ProseMirror-selectednode' : ''}`}
          style={{
            position: 'relative',
            width: '100%',
            maxWidth: `${width}px`,
        ...
        >
          ...
          <p style={{ color: '#14171a', fontWeight: '500', marginBottom: '8px' }}>X/Twitter Post</p>
          <a 
            href={tweetUrl}
            target="_blank"
  ...
  return (
    <NodeViewWrapper className="embed-node-wrapper">
      <div
        ref={containerRef}
        className={`twitter-embed-container ${selected ? 'ProseMirror-selectednode' : ''}`}
        style={{
          display: 'flex',
      ...
      >
        {isLoading && (
          ...
          >
            ...
            />
            <p style={{ color: '#666', fontSize: '14px' }}>Loading tweet...</p>
          </div>
        )}
        
        {/* Official Twitter blockquote embed - same pattern as Instagram */}
        <blockquote
