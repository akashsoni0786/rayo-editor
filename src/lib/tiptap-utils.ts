...
export const handleImageUpload = async (
  ...
  const { uploadToCloudinary, getCloudinaryConfig, fileToBase64 } = await import("../utils/cloudinaryUpload")
...
export function sanitizeUrl(
  ...
  try {
    // Auto-prepend https:// for URLs without a protocol (e.g. "www.example.com")
    // so they don't resolve as relative paths against baseUrl
    let normalizedInput = inputUrl
    if (normalizedInput && !/^[a-zA-Z][a-zA-Z\d+\-.]*:/.test(normalizedInput)) {
      normalizedInput = `https://${normalizedInput}`
    }

    const url = new URL(normalizedInput, baseUrl)
