 ...
 */
export function getPopularCountryCodes(): CountryCode[] {
  const india = countryCodes.find(country => country.code === 'IN');
  const us = countryCodes.find(country => country.code === 'US');
  const uk = countryCodes.find(country => country.code === 'GB');
  const canada = countryCodes.find(country => country.code === 'CA');
  const australia = countryCodes.find(country => country.code === 'AU');
  const germany = countryCodes.find(country => country.code === 'DE');
  const france = countryCodes.find(country => country.code === 'FR');
  const singapore = countryCodes.find(country => country.code === 'SG');
  const uae = countryCodes.find(country => country.code === 'AE');
  const japan = countryCodes.find(country => country.code === 'JP');
  
  return [
    india || { code: 'IN', name: 'India', dial_code: '91' },
    us || { code: 'US', name: 'United States', dial_code: '1' },
    uk || { code: 'GB', name: 'United Kingdom', dial_code: '44' },
    canada || { code: 'CA', name: 'Canada', dial_code: '1' },
    australia || { code: 'AU', name: 'Australia', dial_code: '61' },
    germany || { code: 'DE', name: 'Germany', dial_code: '49' },
    france || { code: 'FR', name: 'France', dial_code: '33' },
    singapore || { code: 'SG', name: 'Singapore', dial_code: '65' },
    uae || { code: 'AE', name: 'United Arab Emirates', dial_code: '971' },
    japan || { code: 'JP', name: 'Japan', dial_code: '81' },
  ];
}
