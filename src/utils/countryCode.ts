// Mapping of ISO 3166-1 alpha-3 to alpha-2 country codes
const alpha3ToAlpha2: { [key: string]: string } = {
  ...
  PRT: 'PT', PRI: 'PR', QAT: 'QA', REU: 'RE', ROU: 'RO', RUS: 'RU', RWA: 'RW', BLM: 'BL',
  SHN: 'SH', KNA: 'KN', LCA: 'LC', MAF: 'MF', SPM: 'PM', VCT: 'VC', WSM: 'WS', SMR: 'SM',
  STP: 'ST', SAU: 'SA', SEN: 'SN', SRB: 'RS', SYC: 'SC', SLE: 'SL', SGP: 'SG', SVK: 'SK',
  SVN: 'SI', SLB: 'SB', SOM: 'SO', ZAF: 'ZA', SGS: 'GS', SSD: 'SS', ESP: 'ES', LKA: 'LK',
  SDN: 'SD', SUR: 'SR', SJM: 'SJ', SWZ: 'SZ', SWE: 'SE', CHE: 'CH', SYR: 'SY', TWN: 'TW',
  TJK: 'TJ', TZA: 'TZ', THA: 'TH', TLS: 'TL', TGO: 'TG', TKL: 'TK', TON: 'TO', TTO: 'TT',
  TUN: 'TN', TUR: 'TR', TKM: 'TM', TCA: 'TC', TUV: 'TV', UGA: 'UG', UKR: 'UA', ARE: 'AE',
  GBR: 'GB', USA: 'US', UMI: 'UM', URY: 'UY', UZB: 'UZ', VUT: 'VU', VEN: 'VE', VNM: 'VN',
  VIR: 'VI', WLF: 'WF', ESH: 'EH', YEM: 'YE', ZMB: 'ZM', ZWE: 'ZW'
};

export function getCountryInfo(alpha3Code: string): { name: string; alpha2Code: string } {
  // Convert to uppercase for consistent lookup
  const alpha3 = alpha3Code.toUpperCase();
  const alpha2 = alpha3ToAlpha2[alpha3];
  
  if (!alpha2) {
    console.warn(`Unknown country code: ${alpha3}`);
    return { name: alpha3Code, alpha2Code: 'US' }; // Fallback to US flag for unknown codes
  }

  // Use Intl.DisplayNames for proper country name localization
  const name = new Intl.DisplayNames(['en'], { type: 'region' }).of(alpha2) || alpha3Code;
  
  return { name, alpha2Code: alpha2 };
}
